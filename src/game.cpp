...
game::game() {
      ...
      for (unsigned char t = 0; t < map_height; t++) {
        ...
        // Pass 7 (create barriers)
          ...
          if (map_temp[i][t] == 1 || map_temp[i][t] == 2 ||
              map_temp[i][t] == 3) {
            barrier newBarrier(&game_world, i * 40, t * 40,
                               blocks[map_temp[i][t]], 0);

            // Destroyable
            if (map_temp[i][t] == 2)
              newBarrier.setHealth(3);
            else if (i != 0 && t != 0 && i != map_width - 1 &&
                     t != map_height - 1)
  ...
  player_tank* newPlayer = new player_tank(
...
void game::update() {
  ...
  for (unsigned int i = 0; i < barriers.size(); i++) {
    if (barriers.at(i).getDead()) {
      ...
      if (random(0, 1) == 0) {
        int type = random(0, 3);
        powerup newPowerup(barriers.at(i).getX(), barriers.at(i).getY(), type,
                           powerup_images[type]);
  ...
  bar_x_1, barriers.at(i).getY() + bar_y_1, barriers.at(i).getX() + bar_x_2,
  barriers.at(i).getY() + bar_y_2, &temp_poi_x, &temp_poi_y)){
