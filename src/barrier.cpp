...
barrier::barrier(world* newWorld,
                 int newX,
                 int newY,
                 BITMAP* newImage,
                 int newHealth) {
...
void barrier::update(std::vector<bullet>* newBullets) {
  if (health > 0 || indestructable) {
    for (unsigned int i = 0; i < newBullets->size(); i++) {
      if (collisionAny(x, x + width, newBullets->at(i).getX(),
                       newBullets->at(i).getX() + 5, y, y + height,
                       newBullets->at(i).getY(),
                       newBullets->at(i).getY() + 5)) {
        if (collisionBottom(
                newBullets->at(i).getY() + newBullets->at(i).getYVelocity(),
                newBullets->at(i).getY() + 5, y, y + height)) {
          newBullets->at(i).reverseDirection("y");
          newBullets->at(i).bounce(BOTTOM);
        }
        if (collisionTop(
                newBullets->at(i).getY(),
                newBullets->at(i).getY() + 5 + newBullets->at(i).getYVelocity(),
                y, y + height)) {
          newBullets->at(i).reverseDirection("y");
          newBullets->at(i).bounce(TOP);
        }
        if (collisionLeft(
                newBullets->at(i).getX(),
                newBullets->at(i).getX() + 5 + newBullets->at(i).getXVelocity(),
                x, x + width)) {
          newBullets->at(i).reverseDirection("x");
          newBullets->at(i).bounce(LEFT);
        }
        if (collisionRight(
                newBullets->at(i).getX() + newBullets->at(i).getXVelocity(),
                newBullets->at(i).getX() + 5, x, x + width)) {
          newBullets->at(i).reverseDirection("x");
...
void barrier::explode(int newX,
  ...
  for (int i = 0; i < newAmount; i++) {
    ...
    } while (getr(new_colour) == 255 && getg(new_colour) == 255 &&
             getb(new_colour) == 255);

    // Make particle
    particle* newParticle =
        new particle(newX, newY, new_colour, -newVelocity, newVelocity,
