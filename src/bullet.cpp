  ...
  if (!pendingErase) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if (incidenceDirection == BOTTOM) {
        newParticle = new particle(x, y, makecol(255, random(0, 255), 0), -5, 5,
                                   0, 3, 1, CIRCLE, 10, EXPLODE);
      } else if (incidenceDirection == TOP) {
        newParticle = new particle(x, y, makecol(255, random(0, 255), 0), -5, 5,
                                   -3, 0, 1, CIRCLE, 10, EXPLODE);
      } else if (incidenceDirection == LEFT) {
        newParticle = new particle(x, y, makecol(255, random(0, 255), 0), -3, 0,
                                   -5, 5, 1, CIRCLE, 10, EXPLODE);
      } else {
        newParticle = new particle(x, y, makecol(255, random(0, 255), 0), 0, 3,
                                   -5, 5, 1, CIRCLE, 10, EXPLODE);
      }
