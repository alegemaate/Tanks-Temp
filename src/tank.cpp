...
tank::tank(world* newWorld,
           ...
           float newSpeed,
           BITMAP* newBaseImage,
           BITMAP* newTurretImage,
           BITMAP* newHurtImage,
           BITMAP* newTreadsImage) {
  x = newX;
  y = newY;
...
void tank::explode(int newX,
  ...
  for (int i = 0; i < newAmount; i++) {
    particle* newParticle = new particle(
        newX, newY, makecol(255, random(0, 255), 0), -newVelocity, newVelocity,
        -newVelocity, newVelocity, 1, CIRCLE, newLife, EXPLODE);
...
void tank::checkCollision(std::vector<bullet>* newBullets) {
  for (unsigned int i = 0; i < newBullets->size(); i++) {
    if (collisionAny(
            x, x + 50, newBullets->at(i).getX(),
            newBullets->at(i).getX() + newBullets->at(i).getXVelocity(), y,
            y + 50, newBullets->at(i).getY(),
            newBullets->at(i).getY() + newBullets->at(i).getYVelocity())) {
      health -= 10;
...
void tank::checkCollision(std::vector<barrier>* newBarriers) {
  ...
  for (unsigned int i = 0; i < newBarriers->size(); i++) {
    if (collisionAny(
            ...
            newBarriers->at(i).getX(),
            newBarriers->at(i).getX() + newBarriers->at(i).getWidth(), y + 2,
            y + height - 2, newBarriers->at(i).getY(),
            newBarriers->at(i).getY() + newBarriers->at(i).getHeight())) {
      canMoveX = false;
    }
    if (collisionAny(
            x + 2, x + width - 2, newBarriers->at(i).getX(),
            newBarriers->at(i).getX() + newBarriers->at(i).getWidth(),
            y + 2 + guess_vector_y, y + height - 2 + guess_vector_y,
            newBarriers->at(i).getY(),
            newBarriers->at(i).getY() + newBarriers->at(i).getHeight())) {
      canMoveY = false;
    }
  }
...
void tank::checkCollision(std::vector<powerup>* newPowerups) {
  for (unsigned int i = 0; i < newPowerups->size(); i++) {
    if (collisionAny(
            x, x + 50, newPowerups->at(i).getX(),
            newPowerups->at(i).getX() + newPowerups->at(i).getWidth(), y,
            y + 50, newPowerups->at(i).getY(),
            newPowerups->at(i).getY() + newPowerups->at(i).getHeight())) {
...
void tank::shoot(float newRotation, float newX, float newY) {
  if (bullet_delay > fire_delay_rate) {
    ...
    bullet newBullet(worldPointer, newX, newY, newRotation, fire_speed, true,
                     1 + num_bullet_bounces + (magicMODE * 10), sample_shot);
...
// Draw health
void tank::drawHealthBar(BITMAP* tempImage,
                         int newX,
                         int newY,
                         int newWidth,
                         int newHeight,
                         int newBorderWidth) {
  // Health Bar
  rectfill(tempImage, newX, newY, newX + newWidth, newY + newHeight,
           makecol(0, 0, 0));
  rectfill(tempImage, newX + newBorderWidth, newY + newBorderWidth,
           newX + newWidth - newBorderWidth, newY + newHeight - newBorderWidth,
           makecol(255, 0, 0));
  rectfill(tempImage, newX + newBorderWidth, newY + newBorderWidth,
           newX + (((float)health / (float)initialHealth) * newWidth) -
               newBorderWidth,
           newY + newHeight - newBorderWidth, makecol(0, 255, 0));
}
...
void tank::putDecal(BITMAP* tempImage) {
  if (!dead && speed > 0)
    rotate_sprite(tempImage, image_treads, x + width / 2, y,
...
*****************/
// Init
player_tank::player_tank(world* newWorld,
                         int newX,
                         int newY,
                         int newHurtTime,
                         int newHealth,
...
*****************/
// Init
ai_tank::ai_tank(world* newWorld,
                 int newX,
                 int newY,
                 int newHurtTime,
                 int newHealth,
